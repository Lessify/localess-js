...
- `localessVite()` from `@localess/react/vite` in `vite.config.ts`, alongside `tanstackStart()` — automates `localessInit()` and component registration for this Vite-based SSR framework
...
| File | Shows |
| --- | --- |
| `vite.config.ts` | `localessVite()` plugin setup |
