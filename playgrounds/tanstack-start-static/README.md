...
Update the Localess config in `vite.config.ts` (`localess({...})`, used for the build) and wherever `prerender.pages` builds its standalone `localessClient` for path enumeration. Replace `origin`, `spaceId`, and `token` (pre-filled against a shared public demo space) in both places with your own.

## Key files

| File | Shows |
| --- | --- |
| `vite.config.ts` | `localess()` plugin setup + `prerender.pages` path enumeration |
