...
- Server-side fetching with `localessClient` (re-exported from `@localess/vue` — `@localess/client` is never imported directly) in `server/api/content.ts`, using a **secret** token — Nuxt guarantees `server/` files never reach the client bundle
- `Localess` plugin installed once (`app/plugins/localess.ts`), with a **public** token, to enable Visual Editor live sync on top of the server-fetched content
...
Edit the `localessClient({...})` options in `server/api/content.ts` (secret token) and the `Localess` plugin options in `app/plugins/localess.ts` (public token) — replace `origin`, `spaceId`, and `token` (pre-filled against a shared public demo space) with your own.
...
| `app/plugins/localess.ts` | `Localess` plugin install with a public token, component registration |
