...
- The same `[...path]` catch-all route as the SSR variant, but pre-rendered: `getStaticPaths()` builds every locale/slug pair from `getLocalessClient().getLinks({ kind: 'DOCUMENT' })`, and the same rich text and locale/theme-toggle UX is baked into static HTML at build
...
| `src/pages/[...path]/index.astro` | `getStaticPaths()` from `getLinks()`, `getContentBySlug`, `LocalessDocument` |
| `src/shared/utils/locales.ts` | `LOCALES` list fed to `getStaticPaths()` |
