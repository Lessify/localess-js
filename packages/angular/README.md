...
> **Removed in v4.** The `download` transform parameter no longer works — use `downloadLink()`. It also carries a non-ASCII asset name in an RFC 5987 `filename*` parameter, so the file saves under its real name.
