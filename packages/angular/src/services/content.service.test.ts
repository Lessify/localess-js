...
describe('LocalessContentService', () => {
  ...
  it('fetches content on the server and writes it to TransferState', async () => {
    const getContentBySlug = vi.fn().mockResolvedValue({ id: 'c1' });
    ...
    expect(transferState.get(makeStateKey<{ id: string }>('ll:content:slug:home'), undefined)).toEqual({ id: 'c1' });
  });
  ...
  it('hydrates from TransferState on the browser without calling the client', async () => {
    ...
    transferState.set(makeStateKey<{ id: string }>('ll:content:slug:home'), { id: 'c1' });
