import { isPlatformServer } from '@angular/common';
import {
  inject,
  Injectable,
  Injector,
  makeStateKey,
  PLATFORM_ID,
  resource,
  ResourceRef,
  runInInjectionContext,
  StateKey,
  TransferState,
} from '@angular/core';
import type { Content, ContentData, ContentFetchParams, Links, LinksFetchParams } from '@localess/client';
import { LocalessClientService } from './client.service';

@Injectable()
export class LocalessContentService {
  private readonly client = inject(LocalessClientService);
  private readonly transferState = inject(TransferState);
  private readonly platformId = inject(PLATFORM_ID);
  private readonly injector = inject(Injector);

  /**
   * Fetches content by slug, hydrating from `TransferState` on the browser when the same slug
   * was already fetched server-side, and falling back to a direct client call otherwise (pure
   * client-side-rendered apps using a public token).
   *
   * Safe to call from anywhere — a field initializer, `ngOnInit`, an event handler — not just
   * from within an injection context, since `resource()` is created using this service's own
   * injector internally.
   */
  contentBySlug<T extends ContentData = ContentData>(
    slug: () => string,
    params?: ContentFetchParams
  ): ResourceRef<Content<T> | undefined> {
    return runInInjectionContext(this.injector, () =>
      resource({
        params: slug,
        loader: ({ params: slugValue }) =>
          this.loadAndHydrate(makeStateKey<Content<T>>(`ll:content:slug:${slugValue}`), () =>
            this.client.getContentBySlug<T>(slugValue, params)
          ),
      })
    );
  }

  /**
   * Fetches content by ID, with the same `TransferState` hydration behavior as {@link contentBySlug}.
   */
  contentById<T extends ContentData = ContentData>(
    id: () => string,
    params?: ContentFetchParams
  ): ResourceRef<Content<T> | undefined> {
    return runInInjectionContext(this.injector, () =>
      resource({
        params: id,
        loader: ({ params: idValue }) =>
          this.loadAndHydrate(makeStateKey<Content<T>>(`ll:content:id:${idValue}`), () => this.client.getContentById<T>(idValue, params)),
      })
    );
  }

  /**
   * Fetches the links map, with the same `TransferState` hydration behavior as {@link contentBySlug}.
   */
  links(params?: LinksFetchParams): ResourceRef<Links | undefined> {
    const key = makeStateKey<Links>(`ll:links:${JSON.stringify(params ?? {})}`);
    return runInInjectionContext(this.injector, () =>
      resource({
        loader: () => this.loadAndHydrate(key, () => this.client.getLinks(params)),
      })
    );
  }

  private async loadAndHydrate<T>(key: StateKey<T>, load: () => Promise<T>): Promise<T> {
    if (this.transferState.hasKey(key)) {
      const value = this.transferState.get(key, undefined as unknown as T);
      this.transferState.remove(key);
      return value;
    }
    const value = await load();
    if (isPlatformServer(this.platformId)) {
      this.transferState.set(key, value);
    }
    return value;
  }
}
