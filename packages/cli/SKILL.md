...
### `schema pull [--path <dir>] [--print-width <n>]`
...
- A `defineField(...)` call with many properties can exceed a readable line length; `--print-width` (default `80`, Prettier's own default — not tied to this CLI's or any particular project's own `.prettierrc`) sets the column threshold before it wraps to one property per line. Set it to match your own project's `printWidth` so pulled files read like the rest of your codebase.
