...
**Current state of the client-import boundary.** This package does *not* yet follow the repo-wide "three sanctioned files" convention (`CLAUDE.md` rule 7: a models module, a utils module, and one client file). Today `@localess/client` is imported directly in:

- `src/index.ts` — the public re-exports (`localessClient`, `LocalessApiError`, `loadLocalessSync`, `localessEditable`, `localessEditableField`, `isBrowser`, `isIframe`, and the sync event types). This is the sanctioned pass-through to consumers.
- `src/models/index.ts` — type re-exports (`EventToApp`, `LocalessClient`). Model types come from `@localess/model` here.
- `src/models/client.ts`, `src/models/localess-component-props.ts`, `src/models/localess-schema-props.ts` — `LocalessClientOptions` and the `Assets`/`ContentData`/`Links`/`References` types (the latter two files import model types from `@localess/client` rather than `@localess/model`).
- `src/lib/localess-integration.ts` and `src/vite-plugins/vite-plugin-localess-init.ts` — `LocalessClientOptions` (type-only).

There is no `localessClient(...)` call in TypeScript source: the client is constructed inside the generated `virtual:localess-init` module, which imports `localessClient` from `@localess/astro`. The `.astro` components import `localessEditable`/`toCamelCase` from `'../'` (the `index.ts` barrel) and types from `'../models'`. Per `CLAUDE.md`, bring the package into compliance when next touching these imports (route `LocalessClientOptions` and the model types through `src/models`, so only `src/index.ts` and the models barrel touch `@localess/client`) rather than as a standalone sweep — and don't add new direct `@localess/client` imports elsewhere in the meantime.
