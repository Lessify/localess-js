...
const VIRTUAL_MODULE_ID = 'virtual:localess-svelte-components';
...
export function generateComponentsModuleCode(componentsDir: string, manualRegistrations: ManualComponentRegistration[]): string {
  const globPattern = `${normalizePath(componentsDir)}/**/*.svelte`;
...
export function vitePluginLocalessComponents(componentsDir: string, components: Record<string, string>): Plugin {
  return {
    name: 'vite-plugin-localess-svelte-components',
...
export { VIRTUAL_MODULE_ID as VIRTUAL_LOCALESS_SVELTE_COMPONENTS_MODULE_ID };
