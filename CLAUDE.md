...
7. **Internal-reference-only imports.** Within a package's `src/`, only that package's dedicated internal re-export module(s) (e.g. `core/models/`, `models.ts`, or a `models/` + `utils.ts` pair) may import from `@localess/client` directly. Every other internal file — including the public entry point (`index.ts` / `public-api.ts`) — imports those types/values through those internal modules instead (e.g. `from '../models'`, `from '../utils'`, never `from '@localess/client'`). This keeps the client-package boundary at a small, fixed set of files per package, so it can be audited or changed in one place. Currently enforced in `@localess/svelte` (`src/lib/models/index.ts` for types and the core client factory, `src/lib/utils.ts` for client-side utility functions); `react`, `vue`, `angular`, `cli`, and `astro` still have files that bypass their internal models module — bring a package into compliance when you're already touching its imports, rather than as a standalone sweep.
